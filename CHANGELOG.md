...
> 且「对方最后一条短消息被 OCR 漏检 → 尾部只剩时间戳」时一条建议都不出。

### Fixed
- **H4 检测器换更稳的算法**:旧版按「最右的亮→暗跨越」找分界,会被**对方头像那种窄亮块**骗到、
  落到名字右边把名字切掉,换个会话列表内容就漂(实测同一窗口 308↔365 跳)。改成「**取最左那条
  够宽亮带(= 会话列表)的右缘**」——头像是窄亮块,天然被忽略,换什么聊天都稳。失败图实测从 365
  修正回 308、title 恢复「陈婕」(顺带更正的裁剪让被漏的「没录」也读出来了)。
- **判分支用最后一条真实消息**(忽略尾部时间戳/系统行):`_one_suggestion` 原来看 `msgs[-1]`,一条
  结尾时间戳就被当成「系统结尾 → 不出建议」;改成取最后一条非系统消息判我方/对方结尾,不再哑火。

### Verified
- 失败截图重测:auto x0 365→308、title=「陈婕」、末条真实发言人=对方 → 正常进回复分支。50 单测全过。

## Unreleased - 2026-06-22 (回复改单框 + 目标模式「追 / 维持关系」)

> 重构:① 回复区从「每次自动出 3 条人设草稿」→「单框,工具栏选一种模式只出一条」;
> ② 模式从**语气**(正式/亲和…用户拿不住)改成**目标**——用户开软件主要就是「不会接感兴趣的人的话」,
> 所以默认「**追**」,另可选「**维持关系**」。策略不写死:交给模型**按军师判定的分数自行拿捏火候**。
> 顺带省钱:回复生成从每次读取 3 次模型调用 → 1 次。

### Changed
- **回复区:3 张人设卡 → 单框**。工具栏新增「**模式**」下拉(仿「模型」选择器):**追(默认)/ 维持关系**;
  选中即按当前对话重出(不重新截图),选择记进 localStorage。去掉「★推荐」排名。
- `agent`:新增 `_MODE_GUIDE`(追/维持的目标指导)+ `mode_guide()`。**追**强调「按分数把握火候,绝不越级、
  也不做有问必答的工具人」(低分先升好感、高分才升温/约见);**维持**只稳温度不加压。`draft_reply` /
  `draft_followup` 的「人设槽」改成「目标槽」。温度 `pursue 0.8 / maintain 0.68`。
- `copilot`:`list_personas` / `_resolve_persona` 改为模式(pursue/maintain,默认 pursue);`_one_suggestion`
  用 `agent.mode_guide(mode)` 取代加载人设文件。`read_and_suggest(persona=)` / `regenerate` 单条出。
  旧 4 语气人设退居内部(不再面向用户)。
- 新增 `GET /api/personas`(返回模式);`/api/read` 接受 `?persona=`;`/api/regenerate` 返回结构化结果。

### Why / Verified
- **成本**:每次读取的回复生成 3 次 → 1 次(叠加 F2 缓存更省)。
- 真机 DeepSeek:**追**低分给带人设的开聊钩子(「在学咖啡拉花,翻车率挺高😂/你周末怎么过?」)、
  不平淡工具人;**维持**放松不加压;我方结尾走 F1 追加判定。**50 单测全过**。
- 已知:追的「低分→高分」升温梯度偏软(DeepSeek 对调情偏保守,合成中性问题也压缩了升温空间);
  真机情绪/暧昧消息上梯度更明显,待真机观察是否要再放开一档。前端单框+下拉待真机点测。

## Unreleased - 2026-06-22 (F1 我方结尾也能给「追加」建议)

> 痛点:真人发完一条后常会追加一句(补充/接话/找补),但旧逻辑只在**对方**结尾才出建议
> (`copilot.py` 的 `msgs[-1].sender not in 我/系统/unknown` 门),我方结尾直接 0 建议。
> 核心不是「造一句话」,是判断「**该不该闭嘴**」——默认偏「先别追」,贴「不舔/点到为止」定位。

### Added
- `agent.draft_followup`:我方结尾(对方没回)时,判断该不该追加 + 给草稿。返回
  `{hold, reason, text}`——`hold=True` 给「先别发」的理由(默认倾向),`hold=False` 给 1~2 条追加。
  规则强约束:发完默认等对方;只在「没说完/有歧义要澄清、能自然递进、上句发崩要找补」时才追;
  绝不催回复(不发「在吗」),军师阶段越低越保守。复用缓存友好 prompt 排序 + 军师判定校准火候。
- `copilot.read_and_suggest`:新增**我方结尾分支**——调 `draft_followup`,`hold` 时在卡片区
  显示「💡 建议先不发 —— 理由」,否则给一张标着「追加 · 可发可不发」的草稿卡(无★推荐、无再生成,
  避免和正常回复混淆)。返回新增 `followup` 字段驱动前端;状态栏标「追加模式」。
- 监控不受影响:`peek` 只在对方发新消息时触发,追加天然是手动读取时才出,工具不会自己催你。

### Verified
- 50 单测全过(+3:hold 带理由 / hold 无分隔符 / 给草稿多行气泡解析)。
- 真机 DeepSeek 跑 7 个情形:刚给完整关心、已约好、催回复等 → 正确 hold(理由合理);
  「发完说错了要找补」→ 给出自然的找补草稿。判断偏保守,符合「默认别追」设定。

## Unreleased - 2026-06-22 (OCR 跨设备健壮性 · H1 截窗口自身像素 PrintWindow)

> 真机暴露的真问题:微信被别的窗口(抖音/IDE/浏览器)盖住时,旧的「截屏幕坐标区域」会**默默截到
> 压在上面的那个窗口**,把抖音评论/IDE 文字当成聊天去读、去生成回复。而「先激活目标窗口」常因
> Windows 前台锁(后台进程不让抢焦点)失败。这是 H1。

### Added / Changed
- `vision._win_grab_pw`:用 Win32 `PrintWindow(PW_RENDERFULLCONTENT=2)` 截目标窗口**自身像素**,
  **无视遮挡、不抢前台**。`grab()` 改为**优先 PrintWindow**,黑屏/失败(老系统、非常规窗口)才回退
  原「激活 + ImageGrab 屏幕区域」。PrintWindow 成功时不再 activate,**不打扰你当前的前台窗口**;
  监控轮询(activate=False)也能稳定截到目标窗口。
- **推翻审计旧结论**:审计原写「PrintWindow 对微信这种 Chromium 内核常黑屏」。真机实测——`flag=0`
  确实黑(均亮 6.2),但 **`flag=2`(PW_RENDERFULLCONTENT)不黑(均亮 46.8),能完整截到微信本身**。

### Why / Verified
- 真机实测:微信**被 IDE 窗口完全压住**、且 `activate=False` 时,新 `grab()` 仍截到 896×648 的
  微信本体,`chat_title='陈婕'`、消息为真实对话(此前同场景会截到抖音评论)。47 单测全过。
- 解决一类最坑的静默错误(读到隔壁窗口当聊天),并消除「手动读取时微信被弹到前台」的打扰。

## Unreleased - 2026-06-22 (OCR 跨设备健壮性 · H4 动态左裁剪)

> 真机暴露:`crop_left=0.40` 固定比例在 896px 深色窗口上切到 x=358,把联系人名(x≈318)和对方头像
> (x≈328)一起削掉 → title=unknown、头像丢失。改成自动检测「会话列表↔聊天区」分界按实际像素裁。

### Changed
- `vision.py` 新增 `_detect_left_boundary`:列表和聊天区底色不同,在合理范围找「亮列表→暗聊天区」
  最右的整带跨越(宽窗排除窄气泡干扰),再吸附到最陡亮度下降沿(真分界线)。`_apply_crop` 用它
  替代 `int(w*crop_left)`。对比太弱(浅色模式/无列表)或检测不到 → 回退固定比例,绝不比原来差。
  新增 `crop_auto` 配置(默认 True)。窗口缩放/换机器自适应,不再像固定比例那样误伤。

### Why / Verified
- 真机(896px 深色窗口):自动落在 **x=308**(真分界,旧固定比例 x=358),**title 恢复为「陈婕」**、
  对方头像回到画面。title 恢复进而让**联系人记忆 + 阶段缓存能正确绑定**(此前 unknown 不缓存)。
- 47 单测全过(+3 边界检测:检到分界 / 弱对比回退 / 关闭用比例);geometry harness 全矩阵 PASS。

### 已知副作用(诚实记录)
- 头像回到画面后,**紧挨头像的极短消息(如 2 字「客人」)偶尔被头像几何判定吞掉**(M1/M2 弱点
  浮现,属另一条线,见 `docs/ocr-robustness-plan.md`)。净收益为正(换来 title + 头像 + 记忆/缓存绑定)。
- 另:易读错的「诶哟」是 easyocr 对「诶」字的识别上限,与本次裁剪无关(已查明)。

## Unreleased - 2026-06-21 (DeepSeek 成本优化 · 第三步:prompt 缓存友好排序)

> 路径见 `docs/product-backlog.md` 的 F2 步骤 3。让静态前缀逐字节稳定,吃满 DeepSeek 自动前缀缓存(命中部分约 1/10 计费)。

### Changed
- `agent.py` `draft_reply`:系统 prompt 重排「**静态在前、易变在后**」。把静态的「输出硬规则」
  从结尾挪进前部固定块(角色 + LoveHelper 方法论 + 硬规则),半静态人设次之,易变的
  (军师判定 / 手动上下文 / 记忆)垫后,当前对话仍在 user。原「硬规则压轴」是迁就本地 7B 对
  结尾敏感,DeepSeek 无此需要,改以前缀缓存优先。

### Verified
- A/B(同一对话连出 3 条人设草稿):旧排序 3 草稿缓存命中 **0%**(硬规则在发散的人设之后,
  进不了公共前缀)→ 新排序 **12%**(硬规则进入公共前缀被后续草稿命中)。注:这是冷缓存快速
  连打的下限,真实跨读取(同人设、分钟级间隔)整段静态+人设前缀都可命中,收益更大。
- **真机回归**:easyocr + DeepSeek 实跑,三条候选风格分明、在设、无客服腔,质量与改前持平。
  44 单测全过。

### Note
- `.gitignore` 补 `token_usage.log` / `stage_cache.json`(运行时本地文件,不入库)。

## Unreleased - 2026-06-21 (DeepSeek 成本优化 · 第二步:军师判定门控)

> 路径见 `docs/product-backlog.md` 的 F2 步骤 2。关系阶段是慢变量,却被每次读取都重算一遍 —— 白烧一次重调用,还让分数随机抖。改成「缓存 + 按需重算」。

### Changed
- `copilot.py`:`assess_stage`(关系阶段判定)从**每次读取必算**改为**带缓存按需重算**。
  新增 `_staged_analysis`:默认复用 `stage_cache.json` 里上次的判定,仅在 ①无缓存
  ②对方新增 ≥4 条 ③命中「分数事件」关键词(约/见面/喜欢/分手/拉黑…)④手动强刷 时才重打
  DeepSeek。标题为 `unknown` 不落缓存(标题没读准时不串到别的联系人);重算失败回退旧判定不丢分。

### Added
- 手动「**重新判定**」按钮(AI 分析卡右上):关系刚有变化时强制刷新阶段;新增 `/api/reassess`
  端点 + `reassess_stage()`,对**已显示对话**强制重算(不重新截图)。
- 3 个门控单测(复用 / 关键词触发 / 新增阈值 / 强刷 / unknown 不缓存)。

### Why / Verified
- **省钱**:多数读取不再触发那次 assess 重调用 —— 真机实测,同一对话第二次读取 **0 次 DeepSeek 调用**。
- **更稳**:分数不再每次重算地随机抖动,军师判定可信度上升。
- 真机首次给出合理判定(20分 有好感 / 中置信 / 轻推);**44 单测全过**。
